...
var switches = []; //Tone switches boolean 0/1
...
var allTheVideos = [], allTheVideoContexts=[], allTheVideoTextures = [];
...
function makeImage(i) {
    ...
    vid.loop = true;
    vid.preload = "auto";
    vid.src = allVideos[i].img;

    allTheVideos.push(vid);

    var vidImage = document.createElement('canvas');
    vidImage.width = 320;
    vidImage.height = 320;

    var vidImageContext = vidImage.getContext('2d');
    vidImageContext.fillStyle = '#ffffff';
    vidImageContext.fillRect(0, 0, vidImage.width, vidImage.height);
    allTheVideoContexts.push(vidImageContext);

    var vidTexture = new THREE.Texture(vidImage);
    vidTexture.minFilter = THREE.LinearFilter;
    vidTexture.magFilter = THREE.LinearFilter;
    vidTexture.format = THREE.RGBFormat;
    vidTexture.generateMipmaps = false;
    vidTexture.wrapS = vidTexture.wrapT = THREE.ClampToEdgeWrapping;
    vidTexture.needsUpdate = true;

    allTheVideoTextures.push(vidTexture);
...
$("div").css('cursor','url(imgs/PCursor.png),auto');
});
...
function init() {
    ...
    var geometry = new THREE.BoxGeometry(cubeWidth,cubeHeight,cubeDepth, true);
...
/////add background plane
    var planeTexture = new THREE.ImageUtils.loadTexture( 'imgs/floor.png' );
    ...
    for (var j = 0; j < instruments.length; j++) {
        ...
        for (var i = 0; i < 32; i++) {
            ...
            var object = new THREE.Mesh(geometry, new THREE.MeshBasicMaterial({
                map: allTheVideoTextures[j]
            }));

            var spacing = 360 / (instruments.length - 1);


            object.position.x = -2000 + ((cubeWidth*14) * i);
            object.position.y = 1000 - ((cubeHeight*14) * j);
    ...
    geo = new THREE.PlaneGeometry(cubeHeight,cubeHeight);
...
var onTouchStart = function(event) {
    ...
    for(var i=0; i<allTheVideos.length; i++){
        // if (!videoIsLoaded) {
        //     videoo.load();
        //     videoIsLoaded = true;
        // }
...
function render() {
    ...
    for(var i=0; i<allTheVideos.length; i++){
        if (allTheVideos[i].readyState === allTheVideos[i].HAVE_ENOUGH_DATA) {
            allTheVideoContexts[i].drawImage(allTheVideos[i], 0, 0);
            // allTheVideoContexts[i].fillRect( 0, 0, allTheVideos[i].width, allTheVideos[i].height );
            if (allTheVideoTextures[i])
                allTheVideoTextures[i].needsUpdate = true;
        }
    }
...
function update(allFFTspectrums) {
    ...
    for (var j = 0; j < instruments.length; j++) {
        ...
        for (var i = 0, len = 32; i < len; i++) {
            ...
            allRows[j][i].scale.z = cubeHeight + 90 * (allFFTspectrums[j][i] / 255);
